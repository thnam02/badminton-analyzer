...
def stroke_metrics_json_path_for(video_path: Path) -> Path:
    """Map outputs/{id}_pose.mp4 -> outputs/{id}_stroke_metrics.json."""
    return video_path.with_name(f"{video_path.stem}_stroke_metrics.json")


def technique_json_path_for(video_path: Path) -> Path:
    """Map outputs/{id}_pose.mp4 -> outputs/{id}_technique.json."""
